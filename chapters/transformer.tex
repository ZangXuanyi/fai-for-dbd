...
\end{equation}
其中，$d_k$是键向量的维度，用于对注意力权重进行缩放，将方差控制到1，从而避免出现梯度消失的问题，具体分析见\cref{sec:scaling_factor}。
...
上述过程中，Q的来源和K、V的来源都不一样。对于编码器而言，Q、K、V都是来自于同一个输入序列；对于解码器而言，Q来自于解码器的输入序列（当前已经生成的文本序列），而K、V来自于编码器的输出序列（整个输入文本序列）。例如，在将法文“Je suis étudiant”翻译为英文“I am a student”的过程中，编码器和解码器中Q、K、V的来源如表\ref{tab:QKV}所示。可以简单记忆成“编码器看自己，解码器看别人”。

\begin{table}
    \centering
    \begin{tabular}{c|ccc}
        \toprule
        组件 & Q & K\& V & \verb|seq_len| \\
        \midrule
        编码器 & Je suis étudiant & Je suis étudiant & 3 \\
        解码器 & 已经生成的英语序列 & 编码器的输出序列 & Q增长， K\&V不变 \\
        \bottomrule
    \end{tabular}
    \caption{编码器和解码器中Q、K、V的来源}
    \label{tab:QKV}
\end{table}
...
\begin{mdframed}[frametitle=例子：缩放对 softmax 分布的定量影响]
...
缩放因子对训练稳定性的贡献不仅体现在前向传播中保持 softmax 的“柔和”，也体现在反向传播中保持梯度的合理量级。回顾我们在线性层一章对梯度下降收敛条件（定理~\ref{thm:gd_convergence}）的讨论：若损失函数的 Lipschitz 常数 $L$ 过大，则学习率 $\eta$ 必须满足 $0 < \eta < 2/L$ 才能保证收敛。自注意力机制中，若内积的方差过大，则 softmax 输出的 Jacobian 矩阵的谱范数会急剧增大，直接推高整个损失函数曲面的 Lipschitz 常数，使得训练变得异常敏感、容易发散。缩放因子 $1/\sqrt{d_k}$ 通过将输入 softmax 的向量的方差标准化，间接控制了注意力层的 Lipschitz 常数，使训练在更大范围内保持稳定。
...
\begin{enumerate}
    \item \textbf{维度灾难}：向量的维度等于词汇量，在典型的大模型场景中这意味着数万维的稀疏向量，存储和计算开销不可接受。
    \item \textbf{语义鸿沟}：任意两个不同 token 的独热向量之间的欧氏距离都是 $\sqrt{2}$，内积都是 0。“猫”和“狗”之间、“猫”和“电脑”之间在数学上毫无区别。模型无法从输入表示中感知到任何词与词之间的语义亲和性。
    \item \textbf{泛化缺失}：模型在训练中学习了“猫喜欢喝牛奶”，但对“狗喜欢喝牛奶”无从得知任何线索，因为“猫”和“狗”在输入空间中毫无关联。
\end{enumerate}
...
\paragraph{正弦位置编码} 在原始 Transformer 论文\cite{vaswani2023attentionneed}中，Vaswani 等人提出了一种优雅的确定性位置编码——正弦位置编码。对于位置 $pos$（从 $0$ 开始）和维度索引 $i$（从 $0$ 到 $d_{\text{model}}-1$），编码值为：
\begin{equation}
...
我们可以用一张表来直观地比较三种基于 Transformer 的预训练架构的根本区别。
...
\begin{table}[htbp]
    ...
    \begin{tabular}{c|p{3.5cm}|p{3.5cm}|p{3.5cm}}
        ...
        \midrule
        组成模块 & 仅编码器（$L$ 层 Transformer 编码器） & 仅解码器（$L$ 层 Transformer 解码器，无交叉注意力） & 编码器 + 解码器（各有若干层，解码器含交叉注意力） \\
...
从表 \ref{tab:bert_gpt_comparison} 中可以看出，选择哪种架构本质上取决于\textbf{对“双向上下文”和“生成能力”之间权衡}的需求。BERT 因双向编码在理解任务上精度极高，但无法直接生成文本；GPT 凭借自回归生成能力可以续写任意长文，但每一时刻只能看见“过去”；编码器-解码器模型则兼具二者，但代价是更高的参数量和训练复杂度。在大模型时代，随着 GPT 系列通过思维链提示等方式在理解任务上逼近甚至超越了 BERT 系列，边界正在变得模糊，但这种结构性的差异依然是理解不同预训练范式作用的根本框架。
...
\begin{itemize}
    \item  用户输入一些内容，模型把这些内容转成 token 序列，然后过一遍词嵌入得到输入的表示矩阵 $X$ 。
    \item 由一个系统进行拼接：“\verb|[BOS]balabala<stop> [agent]|”
    \item GPT 把这个当成自己说过的话，然后以上述内容为输入，预测下一个 token 是什么，如此反复进行自回归生成，直到生成了一个\verb|<stop>| token 为止。
    \item 重复执行以上过程，用户每次输入新消息，就把新的消息拼接在之前的对话历史后面，继续自回归生成。
\end{itemize}

所以对于BERT和T5/BART来说，它们的架构在对话场景下有几个致命缺陷：
\begin{itemize}
