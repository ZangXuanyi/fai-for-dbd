...
\begin{intro}
    \begin{itemize}
        \item 生成模型的本质是分布之间的映射
        \item 生成模型的训练目标是得分匹配
        \item 普通扩散模型、Flow Matching的工作原理与异同
        \item 生成模型的应用
    \end{itemize}
    本章极其艰深，恐有硕士研究生水准，但基本将生成模型讲透——这和Transformer那一节被迫语焉不详是不同的。数学基础较薄弱的同学可以暂时跳过所有的证明过程，先看结论和直觉，后面再回过头来补数学。
\end{intro}
...
\begin{equation}\label{eq:ddpm_forward}
    \tcbhighmath{x_t = \sqrt{\alpha_t}\,x_{t-1} + \sqrt{\beta_t}\,\varepsilon_t}
\end{equation}
...
\end{equation}
再并入 $\alpha_t\alpha_{t-1}\beta_{t-2}$ 项得 $1-\alpha_t\alpha_{t-1}\alpha_{t-2}$，依此类推，并到第 $1$ 步得 $V_t = 1-\bar\alpha_t$。于是我们推出了下面这个东西：
...
\begin{example}{高斯分布的得分}{gaussian_score}
...
\begin{example}{Langevin采样高斯}{langevin_gaussian}
...
\begin{example}{假如得分估计错误}{incorrect_score_estimation}
...
最直接的想法自然是让网络输出 $s_\theta(x)$ 去回归真实得分。自然就有这个定义：
\begin{definition}{显式得分匹配}{explicit_score_matching}
...
于是得到隐式得分匹配目标。
\begin{definition}{隐式得分匹配}{implicit_score_matching}
...
定义下式为去噪得分匹配损失：
\begin{definition}{去噪得分匹配}{denoising_score_matching}
...
\begin{equation}
J_{\text{DSM}}(\theta) = J_{\text{ESM}}^{q_\sigma}(\theta) + \text{const}
\end{equation}
\end{theorem}
...
\begin{theorem}{去噪得分匹配恒等式}{denoising_score_matching_identity}
...
\begin{example}{去噪得分匹配恒等式的例子}{denoising_score_matching_identity_example}
...
\begin{example}{DDPM 的预测噪声就是预测得分的例子}{ddpm_score_example}
...
\begin{framed}{}
\textbf{本章时间约定}：连续时间 $t\in[0,T]$（常取 $T=1$）。$t=0$ 是干净数据，$t=T$ 是纯噪声。注意这与 Flow Matching 的约定相反。
\end{framed}
...
\begin{framed}{}
\textbf{本章时间约定（重要，与前文相反）}：采用 Flow Matching 文献的标准约定。$t\in[0,1]$，\textbf{$t=0$ 是噪声 / 参考分布，$t=1$ 是数据分布}。即 $p_0 = p_{\text{ref}} = \mathcal N(0,I)$，$p_1 = p_{\text{data}}$。

\textbf{务必注意记号 $x_0$ 的含义在此反转}：前文 $x_0$ 指\textbf{干净数据}，而从此处起 $x_0$ 指\textbf{噪声样本}、$x_1$ 才是数据样本。这是 DDPM 文献与 Flow Matching 文献各自的习惯，本讲义为对齐两边文献而保留二者；读到含 $x_0$、$x_1$ 的式子时，请先确认当前属于哪一边，再判断哪个是数据、哪个是噪声。
\end{framed}
...
p_t = \phi_t\# p_0,
\end{equation}
其中 $\#$ 表示分布的推前（pushforward）。这等价于 $\{p_t, v_t\}$ 满足\textbf{连续性方程}：
...
由这些条件路径，定义
\begin{definition}{边缘概率路径}{}
    边缘概率路径 $\{p_t\}$ 定义为对条件路径的积分。
\begin{equation}\label{eq:marginal_probability_path}
p_t(x) = \int p_t(x\mid x_1)\, q(x_1)\, \mathrm{d}x_1,
\end{equation}
其中 $q = p_{\text{data}}$。
\end{definition}
容易验证它满足两端边界：$t=0$ 时 $p_0(x)=\int\mathcal N(0,I)\,q(x_1)dx_1=\mathcal N(0,I)$；$t=1$ 时 $p_1(x)=\int\delta(x-x_1)q(x_1)dx_1=q(x)=p_{\text{data}}$。正合要求。

所以可以得到条件Flow Matching 损失。
\begin{definition}{条件 Flow Matching 损失}{}
对每条条件路径 $\{p_t(\cdot\mid x_1)\}$，其条件速度场 $\{u_t(\cdot\mid x_1)\}$ 定义的条件 Flow Matching 损失为：
